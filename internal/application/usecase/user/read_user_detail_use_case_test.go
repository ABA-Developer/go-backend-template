...
func TestReadDetailUserUseCase(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			userID: "u1",
			setup: func(userRepo *mockrepo.UserRepositoryMock) {
				userRepo.On("ReadUserByIDQuery", testifymock.Anything, "u1").Return(model.User{}, errors.New("db down")).Once()
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := uc.ReadDetailUserUseCase(context.Background(), tt.userID)
			if tt.wantErr != nil {
				assert.Error(t, err)
