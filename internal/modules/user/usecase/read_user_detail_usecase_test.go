...
func TestReadDetailUserUsecase(t *testing.T) {
	logger := zerolog.Nop()
	...
	tests := []struct {
		...
		wantErr     bool
		expectedErr error
		expectedRes domain.UserDetailRow
		setupMock   func(mockRepo *domain.MockUserRepository)
	}{
		{
			name:    "success read user detail",
			userID:  "user-123",
			wantErr: false,
			expectedRes: domain.UserDetailRow{
				ID:       "user-123",
		...
		},
		{
			...
			setupMock: func(mockRepo *domain.MockUserRepository) {
				mockRepo.On("ReadUserByIDQuery", mock.Anything, "user-123").
					Return(domain.UserDetailRow{}, context.DeadlineExceeded).Once()
			},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			res, err := usecase.ReadDetailUserUsecase(ctx, tc.userID)
